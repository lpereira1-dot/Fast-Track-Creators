...
class CreatorIQConfig:
    ...
    Exact endpoint paths and response field names can vary by account/API
    version, so both are configurable here rather than hard-coded -- see
    README.md "Adapting to your CreatorIQ account" for how to tune them.
    """
    ...
    # Endpoint paths (relative to base_url). Defaults match the endpoint
    # names documented at apidocs.creatoriq.com; override if your account
    # uses different paths/versions.
    publishers_path: str = field(
        default_factory=lambda: _env_str("CREATORIQ_PUBLISHERS_PATH", "/v1/publishers")
    )
    activation_report_path: str = field(
        default_factory=lambda: _env_str(
            "CREATORIQ_ACTIVATION_REPORT_PATH", "/v1/reports/publisher-activation"
        )
    )
    activity_report_path: str = field(
        default_factory=lambda: _env_str(
            "CREATORIQ_ACTIVITY_REPORT_PATH", "/v1/reports/publisher-activity"
        )
    )

    # Optional scoping: most CreatorIQ orgs group Fast Track creators under a
    # specific Campaign or Publisher List. Leave blank to pull all publishers.
    ...
    # The JSON key under which each endpoint's list of records lives. The
    # client also falls back to trying "data"/"items"/"results" automatically.
    response_root: str = field(default_factory=lambda: _env_str("CREATORIQ_RESPONSE_ROOT", "data"))
    next_cursor_key: str = field(
        default_factory=lambda: _env_str("CREATORIQ_NEXT_CURSOR_KEY", "next_cursor")
    )
    cursor_param: str = field(default_factory=lambda: _env_str("CREATORIQ_CURSOR_PARAM", "cursor"))
    page_size: int = field(default_factory=lambda: _env_int("CREATORIQ_PAGE_SIZE", 200))
